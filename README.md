...
`learning_rate_reset`：学习率重置，可选择`True`或`False`，若为`True`则在训练新的放大系数时将学习率变为初始学习率，为`False`则接着当前的学习率继续训练新的放大系数，这个参数仅可用于预上采样模型或在前向传播时定义放大系数的后上采样模型，默认为`True`
